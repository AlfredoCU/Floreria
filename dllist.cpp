...
DLNode* DLList::insertData(DLNode *p, Empleado& e) {
...
DLNode *DLList::findData(Empleado& e) {
    DLNode* aux= header->getNext();
    while(aux!= header) {
        if(aux->getData()== e) {
            return aux;
...
Empleado& DLList::retrieve(DLNode* p) {
    if(!isValidPos(p)) {
...
void DLList::writeToDisk(const std::string& fileName) {
    ...
    while(aux != header) {
        ...
        aux->getData().getProyServList().writeToDisk(aux->getData().getEmpleadoNum());
        aux=aux->getNext();
        }
    myFile.close();
    }
...
void DLList::readFromDisk(const std::string& fileName) {
    ...
    while(myFile>>myEmpleado) {
        myPos=insertData(getLastPos(), myEmpleado);
        myPos->getData().getProyServList().readFromDisk(myEmpleado.getEmpleadoNum());
        }
    myFile.close();
    }
